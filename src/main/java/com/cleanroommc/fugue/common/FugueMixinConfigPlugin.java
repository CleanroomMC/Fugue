...
public class FugueMixinConfigPlugin implements IMixinConfigPlugin {
    ...
    public boolean shouldApplyMixin(String targetClassName, String mixinClassName) {
        return switch (mixinClassName.split("\\.")[4]) {
            ...
            case "codechickenlib" ->
                Launch.classLoader.isClassExist("codechicken.lib.CodeChickenLib")
                        && !Launch.classLoader.isClassExist("codechicken.lib.Reference")
                        && FugueConfig.modPatchConfig.enableCCL;
            case "minecraftmultipartcbe" ->
                Loader.isModLoaded("minecraftmultipartcbe") && FugueConfig.modPatchConfig.enableMultiPart;
            ...
            case "gregtech" ->
                Loader.isModLoaded("gregtech")
                        && FugueConfig.modPatchConfig.enableGTCEU
                        && !Fugue.isModNewerThan("gregtech", "2.8.7");
            ...
            case "armourers_workshop" ->
                Loader.isModLoaded("armourers_workshop") && FugueConfig.modPatchConfig.enableArmourersWorkshop;
            ...
            case "refinedstorage" ->
                Loader.isModLoaded("refinedstorage")
                        && FugueConfig.modPatchConfig.enableMoreRefinedStorage
                        && Fugue.isModNewerThan("refinedstorage", "2.0.0");
            case "jei" ->
                Loader.isModLoaded("jei")
                        && FugueConfig.modPatchConfig.enableHEI
                        && Fugue.isModNewerThan("jei", "4.17.0");
            case "carryon" -> Loader.isModLoaded("carryon") && FugueConfig.modPatchConfig.enableCarryon;
            case "betterrecords" ->
                Loader.isModLoaded("betterrecords") && FugueConfig.modPatchConfig.enableBetterRecords;
            case "unilib" -> Loader.isModLoaded("unilib") && FugueConfig.modPatchConfig.enableUnilib;
            case "funkylocomotion" ->
                Loader.isModLoaded("funkylocomotion") && FugueConfig.modPatchConfig.enableFunkyLocomotion;
            case "patchouli" -> Loader.isModLoaded("patchouli") && FugueConfig.modPatchConfig.enablePatchouli;
            case "reccomplex" ->
                Loader.isModLoaded("reccomplex")
                        && FugueConfig.modPatchConfig.enableRecurrentComplex
                        && (!mixinClassName.equals("com.cleanroommc.fugue.mixin.reccomplex.ParametersMixin")
                                || SystemUtils.isJavaVersionAtLeast(JavaVersion.JAVA_22));
